...
<h3>Greater Hyderabad Muncipal Corporation</h3>
<h5>Generated Amount</h5>
<%
boolean a=false;
boolean b=false;
int bal=0,pay=0;
String pti=request.getParameter("pti");
session.setAttribute("pti",pti);
try{
Connection con = databasecon.getconnection();
Statement st = con.createStatement();
String sql="SELECT * FROM   ghmc_bill_g where pti_no ='"+pti+"'"; 
...
Connection con = databasecon.getconnection();
Statement st = con.createStatement();
String sql="SELECT * FROM   ghmc_bill_p where pti_no ='"+pti+"'"; 
ResultSet rs=st.executeQuery(sql);
ResultSetMetaData metadata = rs.getMetaData();
int columnCount = metadata.getColumnCount();
		System.out.println(columnCount);
...
for (String columnName : columns) {
		   ...
		   <a href="ghmc_pay.jsp?cn=<%=columnName%>">Pay</a> <%}
