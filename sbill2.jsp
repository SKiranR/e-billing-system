...
boolean b=false;
int bal=0,pay=0;
String spdc=request.getParameter("spdc");
session.setAttribute("spdc",spdc);
try{
Connection con = databasecon.getconnection();
Statement st = con.createStatement();
String sql="SELECT * FROM   spdc_bill_g where spdc_no ='"+spdc+"'"; 
...
Connection con = databasecon.getconnection();
Statement st = con.createStatement();
String sql="SELECT * FROM   spdc_bill_p where spdc_no ='"+spdc+"'"; 
ResultSet rs=st.executeQuery(sql);
ResultSetMetaData metadata = rs.getMetaData();
int columnCount = metadata.getColumnCount();
		System.out.println(columnCount);
...
for (String columnName : columns) {
		   ...
		   <a href="spdc_pay.jsp?cn=<%=columnName%>">Pay</a> <%}
